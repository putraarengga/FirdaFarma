Public Class FormDataKategori
    Dim databaru As Boolean
    Dim ID As String
    Private Sub Label1_Click(sender As Object, e As EventArgs) Handles Label1.Click

    End Sub

    Private Sub FormDataKategori_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        databaru = False
        IsiGrid()
        TextBox2.Enabled = False

        Dim appPath As String = Application.StartupPath()
        Dim img As New System.Drawing.Icon(appPath + "\drugstore.ico")
        Me.Icon = img
        PictureBox1.ImageLocation = appPath + ("\icons\Medical_Pill.ico")
    End Sub
    Sub IsiGrid()
        bukaDB()
        DA = New Odbc.OdbcDataAdapter("SELECT * FROM tkategori", konek)
        DS = New DataSet
        DS.Clear()
        DA.Fill(DS, "tkategori")
        DataGridView1.DataSource = (DS.Tables("tkategori"))
        DataGridView1.Enabled = True
        With DataGridView1
            .RowHeadersVisible = False
            .Columns(0).HeaderCell.Value = "ID Kategori"
            .Columns(1).HeaderCell.Value = "Kategori"
        End With
    End Sub
    'Sub Bersih()
    '   TextBox2.Text = ""
    'End Sub
    Private Sub IsiTextBox(ByVal x As Integer)
        Try
            ID = DataGridView1.Rows(x).Cells(0).Value
            TextBox2.Text = DataGridView1.Rows(x).Cells(1).Value
        Catch ex As Exception
        End Try
    End Sub
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        Me.Close()
    End Sub
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        Dim simpan As String
        Dim pesan As String
        If Button2.Text = "Tambah" Then
            Button2.Text = "Simpan"
            Button3.Text = "Batal"
            Button4.Visible = False
            TextBox2.Enabled = True
            TextBox2.Text = ""
            TextBox2.Focus()
            databaru = True
        ElseIf Button2.Text = "Simpan" Then    'insert atau update tergantung databaru
            If TextBox2.Text = "" Then Exit Sub
            If databaru Then
                pesan = MsgBox("Apakah anda yakin data ini akan ditambah ke database?", MsgBoxStyle.YesNo, vbInformation)
                If pesan = MsgBoxResult.No Then
                    Exit Sub
                End If
                simpan = "INSERT INTO tkategori(Kategori) VALUES ('" & TextBox2.Text & "')"
            Else
                pesan = MsgBox("Apakah anda yakin data ini akan diupdate ke database?", MsgBoxStyle.YesNo, vbInformation)
                If pesan = MsgBoxResult.No Then
                    Exit Sub
                End If
                simpan = "UPDATE tkategori SET Kategori = '" & TextBox2.Text & "' WHERE IDKategori = '" & ID & "' "
            End If
            jalankansql(simpan)
            DataGridView1.Refresh()
            IsiGrid()

            Button2.Text = "Tambah"
            Button3.Text = "Edit"
            Button4.Visible = True
            TextBox2.Enabled = False
            TextBox2.Text = ""
        End If
    End Sub
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        If Button3.Text = "Edit" Then
            Button2.Text = "Simpan"
            Button3.Text = "Batal"
            Button4.Visible = False
            TextBox2.Enabled = True
            TextBox2.Focus()
            databaru = False
        ElseIf Button3.Text = "Batal" Then
            Button2.Text = "Tambah"
            Button3.Text = "Edit"
            Button4.Visible = True
            TextBox2.Enabled = False
            TextBox2.Text = ""
        End If
    End Sub
    Private Sub jalankansql(ByVal sQL As String)
        Dim objcmd As New System.Data.Odbc.OdbcCommand
        bukaDB()
        Try
            objcmd.Connection = konek
            objcmd.CommandType = CommandType.Text
            objcmd.CommandText = sQL
            objcmd.ExecuteNonQuery()
            objcmd.Dispose()
            MsgBox("Data sudah disimpan", vbInformation)
        Catch ex As Exception
            MsgBox("Tidak bisa menyimpan data ke server" & ex.Message)
        End Try
    End Sub
    Private Sub DataGridView1_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridView1.CellClick
        IsiTextBox(e.RowIndex)
    End Sub

    Private Sub DataGridView1_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridView1.CellContentClick

    End Sub
    Private Sub Button4_Click(sender As Object, e As EventArgs) Handles Button4.Click
        Dim hapussql As String
        Dim pesan As String
        pesan = MsgBox("Apakah anda yakin untuk menghapus data pada server?" + TextBox2.Text, vbExclamation + MsgBoxStyle.YesNo, "Perhatian")
        If pesan = MsgBoxResult.No Then Exit Sub
        hapussql = "DELETE FROM tkategori WHERE IDKategori ='" & ID & "'"
        jalankansql(hapussql)
        DataGridView1.Refresh()
        IsiGrid()
        TextBox2.Text = ""
    End Sub
    Private Sub TextBox1_TextChanged(sender As Object, e As EventArgs) Handles TextBox1.TextChanged
        If TextBox1.Text = "" Then
            DataGridView1.Refresh()
            IsiGrid()
        Else
            DataGridView1.Refresh()
            bukaDB()
            DA = New Odbc.OdbcDataAdapter("SELECT * FROM tkategori WHERE Kategori LIKE '%" & TextBox1.Text & "%'", konek)
            DS = New DataSet
            DS.Clear()
            DA.Fill(DS, "tkategori")
            DataGridView1.DataSource = (DS.Tables("tkategori"))
            DataGridView1.Enabled = True
            With DataGridView1
                .RowHeadersVisible = False
                .Columns(0).HeaderCell.Value = "ID Kategori"
                .Columns(1).HeaderCell.Value = "Kategori"
            End With
        End If
    End Sub

    Private Sub FormDataKategori_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
        If e.KeyCode = Keys.Escape Then
            Me.Close()

        End If
    End Sub
End Class